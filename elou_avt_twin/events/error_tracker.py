...
class ErrorTracker:
    ...
    def evaluate_action(
        ...
        if pending:
            ...
            if self._action_type(action) != current.action_type:
                return self._append_error(
                    "WRONG_ACTION_TYPE",
                    Severity.HIGH,
                    action,
                    current,
                    f"Для оборудования «{current.equipment_id}» выбрано действие «{self._action_type(action)}» вместо «{current.action_type}».",
                )
    ...
    def _matches(self, expected: ExpectedAction, action: OperatorAction) -> bool:
        return (
            ...
            and expected.action_type == self._action_type(action)
