...
export default function InstructorTasksPage() {
  ...
  const scenarios = useAsync<LmsScenario[]>(() => api.lmsMyScenarios(), []);
  ...
  const removeTask = async (t: LmsPracticeTask) => {
    if (!window.confirm(`Удалить задание «${t.title}»?`)) return;
    try {
      await api.lmsDeletePracticeTask(t.id);
  ...
  return (
    ...
    >
      <Card>
        {tasks.length === 0 ? (
          <Empty text="Заданий нет — создайте первое задание" />
        ) : (
                  ...
                  <tr key={t.id}>
                    ...
                    <td>
                      <div className="row" style={{ gap: 6 }}>
                        ...
                        <button className="btn btn-danger" onClick={() => void removeTask(t)}>Удалить</button>
                      </div>
                    </td>
                  </tr>
                ))}
